...
export async function searchByHum(
  ...
  if (embeddingServiceUrl) {
    ...
    // Guard 1: Require enough intervals (~3s of sustained humming)
    const voicedRatio = pitchData.num_frames > 0 ? pitchData.num_voiced / pitchData.num_frames : 0;
    if (pitchData.num_intervals < 30) {
      console.log(`[HumSearch] Too few intervals (${pitchData.num_intervals}, need 30+). Voiced: ${(voicedRatio * 100).toFixed(0)}%`);
      return [];
    }

    // Guard 2: At least 25% of frames must be voiced
    if (voicedRatio < 0.25) {
      console.log(`[HumSearch] Voiced ratio too low (${(voicedRatio * 100).toFixed(0)}%, need 25%+). Likely noise.`);
      return [];
    }

    // Guard 3: Melody contour — worship songs can be narrow, so 3 semitones min
    ...
    const span = queryIntervals.length > 0
      ...
      : 0;
    if (span < 3) {
      console.log(`[HumSearch] Melody span too narrow (${span.toFixed(1)} semitones, need 3+). Not enough contour.`);
      return [];
    }
