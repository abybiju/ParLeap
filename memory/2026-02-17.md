...
## Session: February 2026 — Songs UX, community templates, matcher, latency (doc update)

Documented in MEMORY.md, PROJECT_PLAN.md, and project-context.mdc:

- **Strict metadata + iTunes Auto-Fill**: METADATA RULES in formatSongService; `findSongMetadata()` (iTunes); Wand2 Auto-Fill in song editor.
- **CCLI-only template fetch**: Templates by CCLI only; "Use this format"; SongPreviewCards section labels not counted in content line count.
- **Community save dialog**: Max 3 formats per CCLI; dialog on Create Song; toasts for saved / limit / just create.
- **Similar-line matcher**: Block next-line advance when current vs target similarity ≥ 0.65; advance only via end-of-line.
- **Latency**: Instrumentation, STT_LATENCY_MEASUREMENT.md; stick with ElevenLabs.
- **Hum-to-Search**: Dual-path (YouTube-style + BasicPitch); templates structure-only so no auto-paste on "Use this format".
